...
func (e *appError) Error() string {
	return fmt.Sprintf("appError: %v %q %v", e.err, e.message, e.code)
}
...
func checkFlag(flag bool /* parameter */) error /* return arg */ {

	// If the parameter is false return an appError.
	if !flag {
		return &appError{errors.New("appError"), "checkFlag error", 1}
	}
...
func main() {
	...
	case temporary:
		...
		if e.Temporary() {
			fmt.Println("Don't worry, it's temporary")
		}
	default:
		// Apply the default case and just log the error.
		fmt.Printf("Non-temporary Error: ", err)
